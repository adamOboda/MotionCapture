...
capture = cv2.VideoCapture('PoseVideos/2_output.mp4')
size = (int(capture.get(cv2.CAP_PROP_FRAME_WIDTH)), int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT)))
fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 'x264' doesn't work
out = cv2.VideoWriter('PoseVideos/squat_output.mp4',fourcc, 60.0, size)  # 'False' for 1-ch instead of 3-ch for color
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while capture.isOpened():  #while True:
        ...
        if image is not None:
            frame_ = rescale_frame(image, percent=70)
